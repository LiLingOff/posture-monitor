...
操作方式：空白鍵存檔、ESC結束。視窗會即時繪出偵測到的角點與左右共同角點數量，拍攝完成後會停在最後一幀等待按鍵。資料夾中已有足夠張數時會直接跳過，需要重拍請先清空資料夾。
...
改用真正的雙目對極幾何之後，這個補償係數整個消失了，而非替換成更好的數值——它存在的唯一理由已經不復存在。現在直接取耳朵→肩膀的3D向量，投影到矢狀面後與垂直軸計算帶正負號的夾角，即為 θ_CA；θ_sym 同理，將左右肩連線投影到冠狀面後與水平軸計算夾角。
