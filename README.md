...
| 共同角點數量一直不足 | 先確認上一項。重疊率本身不是問題（這組模組在 700mm 是 96%），所以通常是板子離太遠而顯得太小、光線不足、或板子參數與實際不符 |
...
這組模組的視野重疊其實很充足。60mm 基線配 97° 廣角、兩顆鏡頭近乎平行（`inspect` 的「兩相機夾角」一項 <2° 通過），水平重疊率在 200mm 是 87%、700mm 是 96%。實測的共同角點數從 29 到 63（滿分 63）都有，代表有些影像連整塊板子都兩眼看得到。

真正需要 ChArUco 的理由是**畸變係數需要畫面邊角的資料**。標定板寬 210mm，要在畫面裡夠大才量得準角點，就得靠近到 300mm 上下；而要把它推到畫面四個角落，它必然有一部分落到至少一眼的視野外。一般棋盤格只要缺一角整張就作廢，那些邊角影像就全數拍不到。

這不是理論考量。2026-09-22 之前的標定沒把板子推到邊角，畸變模型在畫面邊緣缺乏約束，量測時的垂直視差是 11~16px（門檻 3px）；重拍一組刻意涵蓋四角的影像之後，角度用到的關鍵點降到 0.85~2.54px。

ChArUco 板在每個白格內嵌一個獨一無二的 ArUco 標記，角點因此擁有獨立ID，左右影像只要有足夠多共同ID的角點就能建立對應關係，不需要整塊板子入鏡。
...
**一份基準只對當次 session 有效。** 2026-09-24 實機：同一個人、同一句指示，相隔九分鐘取兩次「坐正」，θ_CA 基準是 +10.04° 與 −4.17°，差 14.21°，而各自的標準誤差只有 ±0.95° 與 ±0.66°。差的不是量測雜訊，是兩次真的坐得不一樣。對照同一天量到的訊號（前傾相對坐正 22.8°），基準的變異已經是訊號的六成，所以每個 session 開始前都要重取。

基準本身的品質會被檢查兩項。標準誤差超過 2° 時提醒重取，因為這個偏移會固定留在之後每一次判定裡，相對 10° 的門檻已經可觀。另一項是把實測散佈跟該距離與方位下的理論量測雜訊對照，實測明顯較大代表受試者在取基準的過程中動了，多出來的那部分不是雜訊。
