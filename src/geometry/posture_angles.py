"""坐姿角度計算。θ_CA、θ_sym照前作公式的3D版本實作，θ_KA公式未定，維持stub。
...
_VERTICAL_AXIS = np.array([0.0, -1.0, 0.0])  # Y向下為正，所以「上」是-Y
...
def theta_ca(keypoints_3d: PersonKeypoints3D, side: str = "right") -> float:
    ...
    前作用單一45°相機、靠sin(45°)補償透視壓縮才能算這個角度；
    3D三角測量後直接用真實深度算矢狀面投影，不需要這個補償係數了。
